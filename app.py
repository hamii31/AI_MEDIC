...
def main():
    ...
    if model_selection == "Diabetes":
        ...
        hba1c = st.number_input("Enter your HbA1c level:", min_value=0.0, max_value=15.0, value=5.5, step=0.1)
        glucose = st.number_input("Enter your blood glucose level:", min_value=0.0, max_value=500.0, value=140.0, step=1.0)
        ...
        fsh = st.number_input("FSH (mIU/mL):", min_value=0.0, max_value=100.0, value=14.0, step=0.1)
        lh = st.number_input("LH (mIU/mL):", min_value=0.0, max_value=100.0, value=7.0, step=0.1)
        lhfshr = lh / fsh if fsh != 0 else 0  # LH/FSH Ratio calculation
        hip = st.number_input("Hip (inch):", min_value=0.0, max_value=60.0, value=38.0, step=0.1)
        waist = st.number_input("Waist (inch):", min_value=0.0, max_value=60.0, value=33.0, step=0.1)
        whr = waist / hip if hip != 0 else 0  # Waist-Hip Ratio calculation
        tsh = st.number_input("TSH:", min_value=0.0, max_value=70.0, value=3.0, step=0.1)
        amh = st.number_input("AMH:", min_value=0.0, max_value=70.0, value=5.0, step=0.1)
        prl = st.number_input("Prolactin (PRL):", min_value=0.0, max_value=150.0, value=25.0, step=0.1)
        rbs = st.number_input("Random Blood Sugar (RBS):", min_value=0.0, max_value=500.0, value=100.0, step=0.1)
        ...
        l_follicle_count = st.number_input("L Follicle Count:", min_value=0, max_value=50, value=6, step=1)
        r_follicle_count = st.number_input("R Follicle Count:", min_value=0, max_value=50, value=6, step=1)
        endometrium_mm = st.number_input("Endometrium (mm):", min_value=0, max_value=20, value=8, step=1)
        ...
        T3_Resin_Uptake_Percentage = st.number_input("T3 Resin Uptake Percentage:", min_value=0.0, max_value=200.0, value=100.0, step=0.1)  
        Total_Serum_Thyroxine_Isotopic = st.number_input("Total Serum Thyroxine Isotopic:", min_value=0.0, max_value=30.0, value=10.0, step=0.1)  
        Total_Serum_Triiodothyronine_Radioimmunoassay = st.number_input("Total Serum Triiodothyronine Radioimmunoassay:", min_value=0.0, max_value=10.0, value=2.0, step=0.1)  
        Basal_TSH_Radioimmunoassay = st.number_input("Basal TSH Radioimmunoassay", min_value=0.0, max_value=60.0, value=3.0, step=0.1)
        Max_Absolute_Diff_TSH_TRH_Injection = st.number_input("Max Absolute Diff TSH TRH Injection", min_value=0.0, max_value=60.0, value=4.0, step=0.1)
        ...
        if st.button("Predict"):
            ...
            confidence = np.max(probabilities)  # Get the maximum probability for the predicted class)

            class_labels = thyroid_model.classes_  # Get class labels from the model
